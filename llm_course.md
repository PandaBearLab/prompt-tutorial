...
本文并非全部是原创， 我大量参考了吴恩大的[chatGpt提示词课程](https://learn.deeplearning.ai/courses/chatgpt-prompt-eng/lesson/1/introduction)， 英文不错的同学， 推荐去认真学习这门课， 如果只需要快速过一遍， 本文希望能对你有帮助。  

除了prompt 内容的介绍， 我还会重点补充关于prompt 调优的一些经验和技巧， 这些并不高深， 也是我从各方吸取的经验， 其中也有我的一些总结。 

此外， 我还会为这一系列内容， 介绍一些关于AI的工具栈， 介绍一些我个人认为是不错的ai工具。 期望这些能帮到你。 
...
- **原则1：编写清晰明确的指示**
  ...
  - 策略1：使用分隔符清晰地指示输入的不同部分
    ...
    > 输入的所有涉及引用的内容， 用能够识别的符号区分， 有利于大模型的辩识
  
  - 策略2：要求结构化的输出
  
    > 你可以大模型生成你想要的格式的数据， 比如csv格式（一种类似于excel的数据形式）， 这就意味着你可用大模型处理小批的量的数据。 
...
为了能让模型更容易的分辩指令和文本， 最佳实践是将文本用分割符进行区分。 常用的分割符有：
...
正如你所见，LLM 根据给定的输入提示词逐个预测下一个词（也就是标记），从而生成回答。在大多数情况下，模型的输出是与提问相关的，并且完全可用，但是在使用语言模型时需要小心，因为它们给出的回答可能不准确。这种回答通常被称为 AI 幻觉，即 AI 自信地给出一个回答，但是这个回答是错误的，或者涉及虚构的信息。对于依赖 GPT 的用户来说，AI 幻觉可能带来危险。你需要仔细核对并批判性地审视模型的回答。
...
在这节课上，你将生成根据每位客户的评论量身定制的客户服务电子邮件。
